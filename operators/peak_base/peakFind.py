...
def tripleRoll(vol, vec):
    ...
    """
    return np.roll(
        np.roll(np.roll(vol, vec[0], axis=0), vec[1], axis=1), vec[2], axis=2
    )
...
def peakFind2D(image, threshold):
    ...
    positions = np.array(
        np.where(pLarge * image)
    ).T.copy()  # return array as [num,posXY]
...
def peakFind3D(vol, threshold):
    ...
    positions = np.array(
        np.where(pLarge * vol)
    ).T.copy()  # return array as [num,posXYZ]
    return positions.astype(np.int)
...
def enforceMinDist(positions, intensities, minDistance):
    ...
    validPeaks = np.zeros(
        positions.shape, dtype=np.float32
    )  # Will hold intensity,X,Y,Z

    alreadyTested = np.zeros(
        positions.shape[0]
    )  # all peaks are tested unless they were already compared to another peak

    for ii, p0 in enumerate(
        positions
    ):  # ii is the iteration number and p0 is the element
        if not alreadyTested[ii]:
            pointsDist = np.sqrt(
                np.sum((positions - p0) ** 2, axis=1)
            ...
            if closePeaks.any():  # test for any values < minDistance
                closePeaksIndices = np.where(pointsDist < minDistance)[
                    0
                ]  # find all close peaks including self
                peakIntensities = intensities[closePeaksIndices]
                highestPeakIndex = closePeaksIndices[
                    np.argmax(peakIntensities)
                ]  # find the index of the highest peak
                alreadyTested[
                    closePeaksIndices
                ] = 1  # Don't test peaks in the future if already tested
                validPeaks[
                    closePeaksIndices, :
                ] = np.nan  # set all values to nan; cant be integer data set
                validPeaks[ii, :] = positions[
                    highestPeakIndex, :
                ]  # set highest intensity to correct values
            else:
...
def peaksToVolume(peakList, volShape, gaussSigma, gaussSize, indexing="ij"):
    ...
    for pos in peakList:
        ...
        sim_volume[
            pos_loc[0] - sz[0] : pos_loc[0] + sz[0] + 1,
            pos_loc[1] - sz[1] : pos_loc[1] + sz[1] + 1,
            pos_loc[2] - sz[2] : pos_loc[2] + sz[2] + 1,
        ] += gg3
    return sim_volume
...
def peaksToImage(peakList, imShape, gaussSigma, gaussSize, indexing="ij"):
    ...
    for pos in peakList:
        ...
        pos_remain = pos % 1
        gg2 = gaussND.gauss2D(
            Y3D, X3D, pos_remain[0], pos_remain[1], gaussSigma[0], gaussSigma[1]
        )
        sim_image[
            pos_loc[0] - sz[0] : pos_loc[0] + sz[0] + 1,
            pos_loc[1] - sz[1] : pos_loc[1] + sz[1] + 1,
...
def lattice2D_norm(u, v, a, b, origin, num_points):
    ...
    Y2D, X2D = np.meshgrid(
        range(num_points[1]), range(num_points[0]), indexing=default_indexing
    )
...
def remove_xrays(imageOriginal, threshold, size_median_filter=(3, 3)):
    ...
    image_filtered[sub_xrays[0], sub_xrays[1]] = image_med_filter[
        sub_xrays[0], sub_xrays[1]
    ]
...
def refineLattice2D(
    ...
    for ii in range(max_iter):
        if refine_locally is True:
            for jj in range(1, 30):
                ...
                A[:, 1] = (
                    np.round(fraction[0] * ab[0, :]) / fraction[0]
                )  # fill the rest of the values with the abc values
    ...
    if verbose:
        ...
        print("u: {}\nv: {}".format(u, v))
        print(
            "|u|: {0[0]:.4}, |v|: {0[1]:.4} (pixels)".format(
                np.linalg.norm([u, v], axis=1)
            )
        )
        print(
            "angle UV: {:.4}".format(
                np.arccos(np.dot(u / np.linalg.norm(u), v / np.linalg.norm(v)))
                * 180.0
                / np.pi
            )
        )
...
def refineLattice3D(
    or0, u0, v0, w0, pos, fraction=(1, 1, 1), max_iter=30, refine_locally=True
):
...
def generateLatticeFromRefinement(origin, u, v, ab, fraction=(1, 1)):
    ...
    lat[:, 0] = (
        origin[0]
        + np.round(ab[:, 0] * fraction[0]) * (u[0] / fraction[0])
        + np.round(ab[:, 1] * fraction[1]) * v[0] / fraction[1]
    )
    lat[:, 1] = (
        origin[1]
        + np.round(ab[:, 0] * fraction[0]) * (u[1] / fraction[0])
...
def lattice2D_2(u, v, a, b, xy0, numPoints):
    ...
    Y2D, X2D = np.meshgrid(
        range(numPoints[1]), range(numPoints[0]), indexing=default_indexing
    )
...
def fit_peaks_gauss2d(image, peaks, cutOut, init, bounds, remove_edge_peaks=True):
    ...
    for ii, index in enumerate(peaks_sort_values):
        ...
        if (
            ...
            curIm = np.float32(
                image[
                    curX - cutOut : curX + cutOut + 1, curY - cutOut : curY + cutOut + 1
                ]
            )
            ...
            y_error = 1 / np.sqrt(
                curIm_norm.ravel() + 0.00001
            )  # add a small offset to remove divide by zero

            optP2D, optCov2D = opt.curve_fit(
                gaussND.gauss2D_FIT,
                (X2D, Y2D),
                curIm_norm.ravel(),
            ...
            optPointsNaN[ii, :] = np.array(
                [curX + optP2D[0], curY + optP2D[1], optP2D[2], optP2D[3]]
            )  # X,Y,sigma_x,sigma_y
        else:
    ...
    optINaN = ndimage.map_coordinates(
        image.astype("f"), ((optPointsNaN[:, 0]), (optPointsNaN[:, 1])), cval=np.nan
    )
...
def fit_peaks_gauss3d(volume, peaks, cutOut, init, bounds, remove_edge_peaks=True):
    ...
    Y3D, X3D, Z3D = np.meshgrid(
        local_region, local_region, local_region, indexing=default_indexing
    )
    ...
    for ii, index in enumerate(validPeaks_sort_values):
        ...
        if (
            ...
            & (curY > cutOut + 1)
            & (curZ > cutOut + 1)
            & (curX < volume.shape[0] - cutOut)
            & (curY < volume.shape[1] - cutOut)
            & (curZ < volume.shape[2] - cutOut)
        ):

            curVol = np.float32(
                volume[
                    curX - cutOut : curX + cutOut + 1,
    ...
    optINaN = ndimage.map_coordinates(
        volume.astype("f"),
        ((optPointsNaN[:, 0]), (optPointsNaN[:, 1]), (optPointsNaN[:, 2])),
        cval=np.nan,
    )
...
def calculate_unit_cell(image, lattice, u, v, unit_cell_size):
    ...
    sub_lattice = lattice2D(
        uu, vv, 1, 1, (0, 0), (num_u, num_v)
    )  # starts at (0, 0). Then offset for each peak.
    ...
    for ii, peak in enumerate(lattice):
        ...
        ndimage.map_coordinates(
            image2, (cur_XX.ravel(), cur_YY.ravel()), order=3, output=cur_cell
        )
