...
def gauss3DGEN_FIT(
    xyz, x0, y0, z0, sigma_x, sigma_y, sigma_z, Angle1, Angle2, Angle3, BG, Height
):
    """
    ...
    print(
        "Warning: this function is using Fortran ordered array for use in tomviz. Need to test this"
    )
    # Todo: Remove Fortran ordering
    v = np.array(
        [
            xyz[0].reshape(-1, order="F") - x0,
